...
export class Game {
   ...
   */  handlePlayerDeath() {
    if (this.state === 'gameover') {
      console.log('⚠️ Player death already handled, skipping...');
      return;
    }
    ...
    try {
      ...
      if (this.rankingSystem && this.rankingSystem.currentSession) {
          ...
          console.log('📊 Updating ranking stats for player death');
      }
      ...
      this._deathExitTimeout = setTimeout(() => {
        try {
          console.log('⏰ Death timeout expired, exiting to menu...');
          this.exitToMenu();
        } catch (error) {
          console.error('❌ Error in death exit timeout:', error);
      ...
      console.log('✅ Player death handling completed successfully');
      ...
      setTimeout(() => {
        console.log('🆘 Emergency exit due to death handling error');
        window.dispatchEvent(new CustomEvent('game:exitToMenu'));
      }, 1000);
    }
  }
   ...
   */  destroy() {
    ...
    if (this._deathExitTimeout) {
      ...
      console.log('⏰ Cleared death exit timeout');
    }
